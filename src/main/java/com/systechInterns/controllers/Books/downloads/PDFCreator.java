...
public class PDFCreator {
    @EJB
     static BookBeanI bookBeanI;
    private final static String[] HEADER_ARRAY = {"S.No.", "Book Title", "ISBN Number", "Publisher"};
    ...
    public static void addContent(Document document, List<Book> dataObjList) throws DocumentException {
        Paragraph paragraph = new Paragraph();
        paragraph.setFont(NORMAL_FONT);
        createReportTable(paragraph, dataObjList);
        document.add(paragraph);
    }
    private static void createReportTable(Paragraph paragraph, List<Book> dataObjList)
        ...
        int count = 1;
        try {
            List<Book> dataObj = bookBeanI.readAll();
            System.out.println("The size is: " + dataObj.size() + " but the list has: " + dataObj);

        }catch (Exception e){
            e.printStackTrace();
        }
            for (Book dataObject : dataObjList) {
//            for (int i=0; i<dataObjList.size();i++)
                System.out.println("The size is: " + dataObjList.size() + " but the list has: " + dataObjList);
                addToTable(table, String.valueOf(count) + ".");
                addToTable(table, dataObject.getTitle());
                addToTable(table, dataObject.getIsbn());
                addToTable(table, String.valueOf(dataObject.getPublisher().getPublisherName() + " " + dataObject.getPublisher().getPublisherCity() + " " + dataObject.getPublisher().getPublisherCountry()));
//            addToTable(table, String.valueOf(dataObject.getAuthors()));
                count++;
            }
            paragraph.add(table);
        }
